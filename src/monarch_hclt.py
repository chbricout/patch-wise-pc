...
def mutual_information(x1: torch.Tensor, x2: torch.Tensor, num_bins: int, sigma: float):
    ...
    x1p = torch.exp(
        -0.5 * (x1.unsqueeze(2) - bins.view(1, 1, -1)).pow(2) / sigma**2
    )  # (B, K1, n_bin)
    x2p = torch.exp(
        -0.5 * (x2.unsqueeze(2) - bins.view(1, 1, -1)).pow(2) / sigma**2
    )  # (B, K2, n_bin)
    ...
    x12p = (
        ...
        ).reshape(K1, num_bins, K2, num_bins)
        / B
    )
...
def mutual_information_chunked(
    x1: torch.Tensor, x2: torch.Tensor, num_bins: int, sigma: float, chunk_size: int
):
    K = x1.size(1)
    mi = torch.zeros([K, K])
    for x_s in range(0, K, chunk_size):
        ...
        for y_s in range(0, K, chunk_size):
            ...
            mi[x_s:x_e, y_s:y_e] = mutual_information(
                x1[:, x_s:x_e], x2[:, y_s:y_e], num_bins, sigma
            )

    return mi


def chow_liu_tree(mi: np.ndarray) -> np.ndarray:
    K = mi.shape[0]
    G = nx.Graph()
    for v in range(K):
        G.add_node(v)
    ...
    # convert the graph to a predecessors list
    root = nx.center(T)[0]
    clt = nx.bfs_tree(T, root)
    n = T.number_of_nodes()
    predecessors = -1 * np.ones(n, dtype=int)
    for parent, child in clt.edges():
...
def construct_hclt_vtree(x, num_bins, sigma, chunk_size):
    mi = (
        mutual_information_chunked(x, x, num_bins, sigma, chunk_size=chunk_size)
        .detach()
        .cpu()
        .numpy()
    )
